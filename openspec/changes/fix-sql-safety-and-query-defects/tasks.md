...
- [ ] 2.1 Add backtick doubling to `Sql::ident()` so an identifier segment cannot terminate its own quoting; verify the `sql-identifier-safety` escaping scenarios pass in `tests/SqlTest.php`
- [ ] 2.2 Add identifier shape validation to `Sql::ident()`, rejecting anything that is not one or more non-empty `[A-Za-z0-9_]` segments separated by single dots, throwing an `Exception` naming the rejected value; verify the acceptance and rejection scenarios in `tests/SqlTest.php`, including that `a.courseId` and `users.userId` are still accepted
