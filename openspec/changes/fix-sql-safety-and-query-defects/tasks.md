...
- [x] 3.2 Change the empty-array path in `getComparison()` to return the `1 = 0` fragment from `Sql::expandIn()` instead of `IS NULL`, inverting the characterisation test from 1.2; verify the empty-array scenario passes and that the array-containing-null and only-null scenarios are unchanged
- [x] 3.3 Confirm the array paths still bind one parameter per distinct value and that duplicates are bound once; verify with the duplicate-values scenario from 1.2
...
- [x] 4.2 Replace the `$params += $chunk` accumulation in `getArgsSqlAndParams()` and the `$whereParams + $havingParams` union in `getDataWithParams()` with a merge that throws on an overwriting key; verify with a test asserting every placeholder appearing in the generated SQL has a binding, across mixed WHERE and HAVING filters
- [x] 4.3 Run the full suite to confirm no existing test depended on a colliding placeholder name; verify with `vendor/bin/phpunit --no-coverage`
...
- [x] 5.2 Add a test that reuses one `SqlPaginator` across two queries returning different row counts and asserts the item count, total count and page count all describe the second query; verify with `vendor/bin/phpunit --no-coverage --filter Paginator`
- [x] 5.3 Confirm a repeated `setMaxItemsPerPage()` with an unchanged value does not trigger a second `COUNT(*)` query; verify with a test counting prepared statements on the mock PDO used by `tests/RepositoryPdoTest.php`
